...
while read -r line ; do
  case $line in
    SYS*)
      ...
      # time
      time="%{B${color_bgdarkhl} T2} ${icon_clock} %{T1} ${sys_arr[3]}"
    ...
    BAT*)
      ...
      if [[ ${bat_arr[1]} == "C" ]]; then
        ...
        ico="${bat_icons[$(((${bat_arr[0]}*(${#bat_icons[@]}-1))/100))+1]}"
      fi
      bkg=-
      frg="${color_fglight}"
      oln="${bkg}"
      ...
      batamt="%{+u U${oln} B${bkg}} %{F${frg} T2} ${ico} %{T1}${bat_arr[0]}% (${bat_arr[3]} ${bat_arr[4]})  %{-u B-}"
      ;;

    WSP*)
      # I3 Workspaces
      #wsp="%{F${color_fglight} B${color_bgdark} T1}"
      wsp="%{T3} ${icon_wsp} %{T1}%{F${color_fglight} B${color_bgdark} T1}"
      set -- ${line#???}
      while [ $# -gt 0 ] ; do
        case $1 in
         FOC*)
    ...
    WIN*)
      # window title
      title=${line#???}
      title="%{F${color_bgdark} B${color_bgdark} T1}${sep_title}${title} "
      ;;

    SID*)
      ssid="%{F${color_fgdark} B${color_bgdark} T2}  ${icon_ssid}  %{T1}${line#???}"
      ;;
      
  esac

  # And finally, output
  #printf "%s\n" "%{U${color_accent1} l}${wsp}${title} %{r}%{A:${act_wifi_c}:}${ssid}%{A}${stab}%{A:${act_cpu_c}:}${cpu}%{A}${stab}%{A:${act_mem_c}:}${mem}%{A}${stab%{A:${act_vol_c}:}${vol}%{A}${stab}%{A:${act_bat_c}:}${batamt}%{A}%{A:${act_date_c}:}${date}%{A}${stab}%{A:${act_time_c}:}${time}%{A}"
  printf "%s\n" "%{U${color_accent1} l}${wsp}%{r}%{A:${act_wifi_c}:}${ssid}%{A}${stab}%{A:${act_cpu_c}:}${cpu}%{A}${stab}%{A:${act_vol_c}:}${vol}%{A}${stab}%{A:${act_bat_c}:}${batamt}%{A}%{A:${act_date_c}:}${date}%{A}${stab}${time} "
done
