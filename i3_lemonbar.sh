...
# get win name
#xprop -spy -root _NET_ACTIVE_WINDOW | sed -un 's/.*\(0x.*\)/WIN\1/p' > "${panel_fifo}" &
xprop -spy -root _NET_ACTIVE_WINDOW | grep '_NET_WM_NAME' | cut -d'"' -f2 > "${panel_fifo}" &
...
while :; do
  ...
  if [ $((cnt_bat++)) -ge ${upd_bat} ]; then
    ...
    #echo "$(upower -i /org/freedesktop/UPower/devices/battery_BAT0 | grep time | cut -d" " -f13-15)" > "${panel_fifo}" &
    cnt_bat=0
  fi
...
cat "${panel_fifo}" | $(dirname $0)/i3_lemonbar_parser.sh \
  | lemonbar -p -f "${font}" -f "${iconfont}" -f "${iconfont2}" -g "${geometry}" -B "${color_back}" -F "${color_fore}" -u 3 | while read line; do eval $line; done
